...
class MotivosPendenciaService implements BaseService<MotivoPendencia> {
    ...
    async getAll(params?: Record<string, string | number | boolean>): Promise<MotivoPendencia[]> {
        return await api.get<MotivoPendencia[]>(this.baseUrl, { params });
    }

    async getAllWithInactive(): Promise<MotivoPendencia[]> {
        return await api.get<MotivoPendencia[]>(this.baseUrl, { params: { incluir_inativos: 'S' } });
    }
    ...
    async update(id: number | string, data: Partial<MotivoPendencia>): Promise<MotivoPendencia> {
