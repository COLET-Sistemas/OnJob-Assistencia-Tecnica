...
class MotivosAtendimentoService implements BaseService<MotivoAtendimento> {
    ...
    async getAll(params?: Record<string, string | number | boolean>): Promise<MotivoAtendimento[]> {
        return await api.get<MotivoAtendimento[]>(this.baseUrl, { params });
    }

    async getById(id: number | string): Promise<MotivoAtendimento> {
        return await api.get<MotivoAtendimento>(`${this.baseUrl}/${id}`);
    }

    async create(data: Omit<MotivoAtendimento, 'id'>): Promise<MotivoAtendimento> {
        return await api.post<MotivoAtendimento>(this.baseUrl, data);
    }

    async update(id: number | string, data: Partial<MotivoAtendimento>): Promise<MotivoAtendimento> {
