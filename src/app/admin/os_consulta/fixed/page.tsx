...
const statusMapping: Record<number, { label: string; className: string }> = {
  1: {
    label: "Pendente",
...
const ConsultaOSPage: React.FC = () => {
  ...
  const mapToOrdemServico = (item: ApiItem): OrdemServico => {
    return {
      ...
      status: item.status || (item.situacao_os?.codigo as number) || 0,
      status_descricao:
  ...
  const handlePrintOS = (id: number) => {
    // Abre uma nova aba com a visualização de impressão
    window.open(`/admin/os_consulta/imprimir/${id}`, "_blank");
  };
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center overflow-y-auto bg-black bg-opacity-50">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-5xl max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between p-6 border-b border-gray-200">
              ...
              <div className="flex items-center gap-3">
                <button
                  onClick={() => handlePrintOS(osDetalhada.id)}
                  className="flex items-center gap-2 px-4 py-2 bg-indigo-50 text-indigo-600 rounded-lg hover:bg-indigo-100 transition-colors border border-indigo-100"
                >
                  <Printer size={16} />
                  <span>Imprimir</span>
